...
CAUTION: When adding a new template, make sure to run the `backend-leocloud-beeyond/src/main/resources/templates/create-file-list.sh` script to update the `file-list.txt` file which is read in the `TemplateBean` class at startup.
